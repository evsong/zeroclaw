...
pub struct LarkChannel {
    ...
    mention_only: bool,
    /// Lark (international) or Feishu (CN) platform selection.
...
impl LarkChannel {
    ...
    fn new_with_platform(
        ...
        let use_feishu = platform == LarkPlatform::Feishu;
        Self {
            app_id,
            app_secret,
            verification_token,
    ...
    /// Build from `LarkConfig` (international Lark endpoints).
    ...
    pub fn from_feishu_config(config: &crate::config::schema::FeishuConfig) -> Self {
        let mut ch = Self::new_with_platform(
            ...
            config.port,
            config.allowed_users.clone(),
            false, // FeishuConfig has no mention_only
            LarkPlatform::Feishu,
        );
        ch.receive_mode = config.receive_mode.clone();
        ch
