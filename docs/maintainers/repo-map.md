...
Tool categories:
- **File/Shell**: `shell`, `file_read`, `file_write`, `file_edit`, `apply_patch`, `glob_search`, `content_search`, `process`
...
- **Delegation**: `delegate` (inline sub-agent runs), `child_session` (bounded async delegated runs), `composio` (OAuth integrations)
